        ...
        match &field.ty {
            Type::Map { key, value } => {
                ...
                let resolved =
                    resolve_const_expr(init, &const_exprs, &mut Vec::new()).map_err(|message| {
                        CodegenError::Unsupported(format!(
                            "contract map field `{}` initializer {message}",
                            field.name
                        ))
                    })?;
                ...
                for (key_expr, value_expr) in pairs {
                    let key = coerce_const_value(key_expr, key)?;
                    let value = coerce_const_value(value_expr, value)?;
            ...
            field_ty => {
                let value_expr = match &field.init {
                    Some(init) => {
                        ...
                        coerce_const_value(resolved, field_ty).map_err(|message| {
                            CodegenError::Unsupported(format!(
                    ...
                    None => coerce_const_value(default_value_for_type(field_ty)?, field_ty)?,
                };
...
fn default_value_for_type(ty: &Type) -> Result<Expr, CodegenError> {
    ensure_initializable_scalar_type(ty)?;
    Ok(match ty {
...
fn coerce_const_value(expr: Expr, ty: &Type) -> Result<Expr, CodegenError> {
    ensure_initializable_scalar_type(ty)?;
    Ok(Expr::Cast {
        expr: Box::new(expr),
        ty: ty.clone(),
...
fn ensure_initializable_scalar_type(ty: &Type) -> Result<(), CodegenError> {
    match ty {
        Type::Bool | Type::Int | Type::String | Type::Hash160 | Type::Hash256 | Type::Buffer => {
            Ok(())
        }
        Type::Array(_) => Err(CodegenError::Unsupported(
            "contract cannot have array fields".to_string(),
        )),
        Type::Map { .. } => Err(CodegenError::Unsupported(
            "contract map field initializer must be a map literal".to_string(),
        )),
        Type::Void | Type::Any | Type::Named(_) => Err(CodegenError::Unsupported(format!(
            "contract mutable field type `{ty}` cannot be initialized automatically"
        ))),
    }
}
